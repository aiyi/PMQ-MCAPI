...
int main()
{
    ...
    //we are the magenta
    mcapi_initialize( id_mag_cyan_send.domain_id, id_mag_cyan_send.node_id, 0,
    0, &info, &status );
    check( MCAPI_SUCCESS, status );

    //create our sending channel endpoint
    mag_cyan_send = mcapi_endpoint_create( id_mag_cyan_send.port_id, &status );
    check( MCAPI_SUCCESS, status );
    //get their receiving channel endpoint
    mag_cyan_recv = mcapi_endpoint_get( id_mag_cyan_recv.domain_id,
    id_mag_cyan_recv.node_id, id_mag_cyan_recv.port_id, TIMEOUT, &status );
    check( MCAPI_SUCCESS, status );

    //create our receiving channel endpoint
    cyan_mag_recv = mcapi_endpoint_create( id_cyan_mag_recv.port_id, &status );
    check( MCAPI_SUCCESS, status );
    //get their sending channel endpoint
    cyan_mag_send = mcapi_endpoint_get( id_cyan_mag_send.domain_id,
    id_cyan_mag_send.node_id, id_mag_cyan_send.port_id, TIMEOUT, &status );
    check( MCAPI_SUCCESS, status );
